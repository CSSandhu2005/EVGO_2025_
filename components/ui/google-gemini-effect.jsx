...
import { NavbarLogo } from "./resizeable-navbar";
...
export const GoogleGeminiEffect = ({
  ...
  return (
    <div className={cn("sticky top-80 w-full", className)}>
      <p className="text-2xl md:text-7xl font-normal pb-1 text-center bg-clip-text text-transparent bg-gradient-to-b from-neutral-100 to-neutral-300">
        {title || `Google Gemini EV Analysis`}
      </p>
      <p className="text-lg md:text-xl font-normal text-center text-neutral-400 max-w-lg mx-auto leading-4.5 md:leading-6">
        {description ||
          `Just UpLoad Your EV Hero Photo, And Let Google Gemni Determine Your Electric Vehice Value.`}
      </p>

      <div className="w-full h-[890px] -top-60 md:-top-40 flex items-center justify-center absolute">
        <button className="font-bold bg-none rouned-full md:px-4 md:py-2 px-2 md:mt-2 mt-8 z-30  text-black text-xs w-fit mx-auto">
          <Image
            width="70"
            height="70"
            src={Logo_Dark[0].image}
            alt="Dark_EVGO_Logo"
            className="border-2 rounded-xl mt-12 md:mt-22"
          />
        </button>
      </div>
      ...
      >
        ...
        {/* Blurred static paths underneath for glow effect */}
        {paths.map((path, idx) => (
          <path
            key={`blur-${idx}`}
            d={path.d}
